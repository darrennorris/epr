...
# (PART) Escala e padrões {.unnumbered}
...
Para que a Ecologia da Paisagem gere evidências científicas robustas e úteis, a escala nas três dimensões deve ser consistente e apropriada para o estudo. Incompatibilidade entre a escala de análise (definida por o pesquisador) e a escala de efeito (definida pelo verdadeiro relacionamento) representa uma versão do Problema de Unidade Areal Modificável ("Modifiable Areal Unit Problem" [@gehlke1934certain]), em que os resultados e a inferência torna-se um artefato do esquema de amostragem espacial em vez de um processo ecológico [@jelinski1996modifiable; @WU2006].
...
Isso nos mostra informações sobre os atributos da raster no objeto ramostra. As informações como escala espacial (resolução e extensão) e a sistema de coordenadas (SIRGAS 2000 / UTM zone 22N , [EPSG:31976](https://epsg.io/31976)) são apresentados e representem os componentes conforme a proxima figura.

```{r raster-components, echo = FALSE, fig.align = 'center', out.width = "60%", out.height = "60%", fig.cap=" Componentes de uma raster e suas atributos no pacote terra."}
knitr::include_graphics(path = "figures/raster_components.png")
```
...
Exemplos : [Pacotes ggplot2 e tmap](https://www.youtube.com/watch?v=OYON-VzPFmw)
...
```{r raster-res, echo = FALSE, fig.align = 'center', out.width = "75%", out.height = "75%", fig.cap=" Comparação de tamanhos de pixels pequenos e grandes. Mostrando diferenças causadas pela agregação de 4 pixels em 1."}
knitr::include_graphics(path = "figures/pixel_size.png")
```
...
buffer.forest1.16000m <- terra::crop(floresta_2020, rio_pontos_31976_b16000, 
                              snap = "out", mask = TRUE)
...
buffer.forest1.250m <- terra::crop(floresta_2020, rio_pontos_31976_b250,
                                  snap="out", mask = TRUE)
# 500 m
rio_pontos_31976_b500 <- st_buffer(rio_pontos_31976[1, ], dist = 500)
buffer.forest1.500m <- terra::crop(floresta_2020, rio_pontos_31976_b500, 
                            snap = "out", mask = TRUE)
# 1000 m
rio_pontos_31976_b1000 <- st_buffer(rio_pontos_31976[1, ], dist = 1000)
buffer.forest1.1km <- terra::crop(floresta_2020, rio_pontos_31976_b1000, 
...
# 2000 m
rio_pontos_31976_b2000 <- st_buffer(rio_pontos_31976[1, ], dist = 2000)
buffer.forest1.2000m <- terra::crop(floresta_2020, rio_pontos_31976_b2000, 
                                    snap = "out", mask = TRUE)
# 4000 m
rio_pontos_31976_b4000 <- st_buffer(rio_pontos_31976[1, ], dist = 4000)
buffer.forest1.4000m <- terra::crop(floresta_2020, rio_pontos_31976_b4000, 
                                    snap = "out", mask = TRUE)
...
`r kableExtra::text_spec("A modelagem multiescala quantifica as condições do ambiente em múltiplas escalas alterando o resolução ou a extensão da análise e, em seguida, avaliando qual das escalas consideradas  explica melhor um padrão ou processo. Escolha 1 espécie aquático e 1 espécie terrestre que ocorram na região a montante das hidrelétricas no Rio Araguari (especificando nome comum junto com o nome científico para ambos). Com base nas diferenças entre extensões (indicados no exemplo anterior) e as características funcionais das espécies (por exemplo área de vida), escolher as extensões mais adequadas para um estudo multiescala de cada espécie.", color = "blue", bold = TRUE)`

------------------------------------------------------------------------

[Soluções dos exercícios:](https://darrennorris.github.io/eprsol/solutions01_escala.html) https://darrennorris.github.io/eprsol/solutions01_escala.html
