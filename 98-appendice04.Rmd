...
# Landscape metrics
mfile <- "C:\\Users\\user\\Documents\\Articles\\2022_Michalski_interview\\analysis\\metric_medians_new.csv"
...
```{r tidy-metrics-forest}
metrics_keep <- c("cpland_0.125", "cpland_0.25", "cpland_0.5", "cpland_1",
                  "cpland_2", "cpland_4", "cpland_8", "cpland_16")
# make final data
# add year to counts
interview_counts |> 
            left_join(interviews |> 
  select(aid, Year, Latitude, Longitude, Habitat_WWF)) |> 
# add metric namess to make full dataset
  crossing(buff_metric = metrics_keep) |>
# add metrics
  left_join(metrics_int |> 
              filter(year_group != 1986, class == 1) |> 
              mutate(buff_metric = paste(metric, buff_dist_km, sep="_")) |>
              filter(buff_metric %in% metrics_keep), 
            join_by(aid, buff_metric, Year == year_group)) |> 
# fill in NAs introduced by join
  mutate(value_median = replace_na(value_median, 0), 
         value_min = replace_na(value_min, 0), 
         value_max = replace_na(value_max, 0),
         metric = replace_na(metric, "cpland"), 
         Descricao = replace_na(Descricao, "Floresta"),
         Description = replace_na(Description, "Forest"), 
         class = replace_na(class, 1),
         pa = ifelse(int_pres > 0, 1, 0)) |> 
  #filter(is.na(buff_dist_km), aid ==55) |> 
  separate_wider_delim(buff_metric, delim = "_", 
                       names = c("a", "buff"),
                       cols_remove = FALSE) |> 
  mutate(buff_dist_km = coalesce(buff_dist_km, as.numeric(buff))) |>  
  select(aid, Latitude, Longitude, Habitat_WWF, class,
         Description, metric, buff_dist_km, buff_metric, value_median,  
         sp, int_pres, pa) -> species_interviews

species_interviews |> 
  filter(is.na(buff_dist_km)) |> 
  group_by(aid) |> summarise(count = n())

```


Plot to compare species.
```{r compare-species}
# abundances
species_interviews |> 
...
png("figures/fig_sp01.png", bg = "white", type = c("cairo"), 
    width=16, height=20, units = "in", res = 600)
fig_sp01
dev.off()
...
species_interviews |> 
  rename(class_description = Description, 
         ...
         sp_ab = int_pres, 
         sp_pa = pa) |>
write.csv("data/species_interviews_forest.csv", 
          row.names = FALSE)
```
