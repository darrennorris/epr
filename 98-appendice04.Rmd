...
Load data (made in 98-appendice03.Rmd and 98-appendice03.Rmd).
```{r load-data}
...
metrics_int |> 
  filter(year_group != 1986, class == 1) |> 
  mutate(buff_metric = paste(metric, buff_dist_km, sep="_")) |> 
    filter(buff_metric %in% c("cpland_1", "cpland_10")) |>
...
png("figures/fig_sp01.png", bg = "white", type = c("cairo"), 
    width=5, height=20, units = "in", res = 600)
fig_sp01
dev.off()
...
left_join(interview_counts |> 
  ...
  select(aid, Latitude, Longitude, Habitat_WWF, 
         metric, buff_dist_km, buff_metric, value_median,  
         sp, int_pres) |> 
