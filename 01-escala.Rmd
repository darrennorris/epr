...
Para carregar um arquivo raster trabalhamos com o pacote [terra](https://CRAN.R-project.org/package=terra). O pacote tem varios funçoes para a análise e modelagem de dados geográficos. Nós podemos ler os dados de cobertura da terra no arquivo ".tif" com a função `r kableExtra::text_spec( "rast", background = "#d3d3d3")`.
...
Podemos também verificar informações sobre o raster (metadados) - simplesmente rodando o nome do objeto:
...
`r kableExtra::text_spec("Com base nos resultados obtidos até agora em relação ao objeto raster ramostra, qual o área do pixel em metros quadrados? Qual o área total da paisagem (raster ramostra) em hectares e quilometros quadrados?", color = "blue", bold = TRUE)`
...
#| message: FALSE
#| warning: FALSE
meuSIG <- system.file("vector/rivers.gpkg", package = "eprdados")
fs <- file.size(meuSIG)/(1024^2)
```

Precisamos carregar os dados para rios e pontos de amostragem (arquivo tipo vector, formato "GPKG", tamanho `r round(fs,1)` MB). Este arquivo contém diferentes camadas vetoriais usadas para avaliar impactos de barragens hidroelétricas em tracajas [@bárcenas-garcía2022] e ariranhas [@raffo2022].
...
O formato "GPKG" é diferente de "tif" (raster), o processo de importação é, portanto com funções diferentes. Primeira, avisar R sobre onde ficar o arquivo. O código abaixo vai fazer isso:
