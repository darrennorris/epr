...
-   Uma métrica de paisagem cpland foi calculada no entorno das propriedades seguindo a metodologia no capítulo \@ref(cap02) usando o pacote [landscapemetrics](https://r-spatialecology.github.io/landscapemetrics/).

As três espécies têm respostas contrastantes à cobertura florestal. Os padrões de resposta seguem aqueles conhecidos para espécies de carnívoros [@MICHALSKI2005383; @MICHALSKI2007conbio].
...
-   $Y_i$ denota a variável de resposta (ou dependente) Y para cada linha $i$; na regressão logística isso é esta é uma variável binária
...
eprdados::carnivoros |> 
  ...
  labs(x = "ocorrência", 
       y = "Área central de floresta\n(porcentagem da paisagem)") -> fig_carnivoros_box
...
png("figures/fig_spp.png", width=7, height=7, 
    units="in", res = 600)
fig_spp
invisible(dev.off())
